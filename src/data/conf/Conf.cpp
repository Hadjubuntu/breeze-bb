...
Conf::Conf()
{
	...
	_parameters.push_back(Param<float>("commandNmToSignalUs", 35.0f));

	// Flight stabilization with simple PID
	_parameters.push_back(Param<float>("flightStabilization_Kangle", 4.75f));
	_parameters.push_back(Param<float>("flightStabilization_KrateRoll", 1.2f));
	_parameters.push_back(Param<float>("flightStabilization_KratePitch", 1.2f));
